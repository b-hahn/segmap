...
def init_model_no_rl(input_shape, n_classes, use_color, use_semantics):
    with tf.name_scope("InputScope") as scope:
        cnn_input = tf.placeholder(
            dtype=tf.float32, shape=(None,) + input_shape + (1,), name="input"
        )
    ...
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        train_op = optimizer.minimize(loss_c, name="train_op")
...
def init_model_color_semantics_no_rl(input_shape, n_classes, use_color, use_semantics):
    with tf.name_scope("InputScope") as scope:
        cnn_input = tf.placeholder(
            dtype=tf.float32, shape=(None,) + input_shape + (4,), name="input"
        )
