...
def init_model_color_semantics(input_shape, n_classes, use_color, use_semantics):
    with tf.name_scope("InputScope") as scope:
        # if use_color:
        #     cnn_input = tf.placeholder(
        ...
        # cnn_input = tf.cond(tf.constant(use_color, dtype=tf.bool),
        #                     lambda: tf.placeholder(dtype=tf.float32, shape=(None,) + input_shape + (4,), name="input"),
        #                     lambda: tf.placeholder(dtype=tf.float32, shape=(None,) + input_shape + (1,), name="input"))
