...
namespace segmatch {
  ...
  static std::map<std::vector<uint8_t>, uint8_t> color_to_class_id = {
  ...
  static std::map<uint8_t, std::vector<uint8_t>> class_id_to_color = {
...
static uint8_t compute_color_to_class_id(uint32_t rgb)
{
    uint8_t r, g, b;
    r = (rgb >> 16) & 0xff;
    g = (rgb >> 8) & 0xff;
