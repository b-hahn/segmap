...
void CNNDescriptor::describe(SegmentedCloud* segmented_cloud_ptr, bool use_color, bool use_semantic_segmentation) {
    if (use_color) {
        // tf_graph_executor::Array4D nn_input;
        describe4D(segmented_cloud_ptr);
    } else {
        describe3D(segmented_cloud_ptr);
    }
}


void CNNDescriptor::describe4D(SegmentedCloud* segmented_cloud_ptr) {
  ...
  for (std::unordered_map<Id, Segment>::iterator it = segmented_cloud_ptr->begin();
    ...
    tf_graph_executor::Array4D nn_input(n_voxels_x_dim_, n_voxels_y_dim_, n_voxels_z_dim_, 4);
    for (const auto& point: rescaled_point_cloud.points) {
      ...
      if (ind_x >= 0 && ind_x < n_voxels_x_dim_ &&
        ...
        if (nn_input.container[ind_x][ind_y][ind_z][0] == 0.0) {
          ++n_occupied_voxels;
        }
  ...
  if (!batch_nn_input.empty()) {
    ...
    if (batch_nn_input.size() < mini_batch_size_) {
      graph_executor_->batchFullForwardPass<tf_graph_executor::Array4D>(batch_nn_input,
                                            kInputTensorName,
                                            scales_as_vectors,
                                            kScalesTensorName,
                                            kFeaturesTensorName,
      ...
      for (size_t i = 0u; i < batch_nn_input.size(); ++i) {
        ...
        if (mini_batch.size() == mini_batch_size_) {
          ...
          graph_executor_->batchFullForwardPass<tf_graph_executor::Array4D>(mini_batch,
                                                kInputTensorName,
                                                mini_batch_scales,
                                                kScalesTensorName,
                                                kFeaturesTensorName,
...
void CNNDescriptor::describe3D(SegmentedCloud* segmented_cloud_ptr) {
  CHECK_NOTNULL(segmented_cloud_ptr);
  constexpr double kMinChangeBeforeDescription = 0.1; // 0.2
  ...
  if (!batch_nn_input.empty()) {
    ...
    std::vector<std::vector<float> > semantics;
    if (batch_nn_input.size() < mini_batch_size_) {
      graph_executor_->batchFullForwardPass<tf_graph_executor::Array3D>(batch_nn_input,
                                            kInputTensorName,
                                            scales_as_vectors,
                                            kScalesTensorName,
                                            kFeaturesTensorName,
      ...
      for (size_t i = 0u; i < batch_nn_input.size(); ++i) {
        ...
        if (mini_batch.size() == mini_batch_size_) {
          ...
          graph_executor_->batchFullForwardPass<tf_graph_executor::Array3D>(mini_batch,
                                                kInputTensorName,
                                                mini_batch_scales,
                                                kScalesTensorName,
                                                kFeaturesTensorName,
